...
function EnrollmentReport() {
    ...
    useEffect(() => {
        const fetchData = () => {
            ...
            fetch("https://kaizenvoiz.ai/dashboardwebapi/api/Dashboard/overallEnroll-checkin-success-report")
                .then((res) => res.json())
                .then((raw) => {
                    ...
                    const formatted = raw.map((item) => ({
                        ...
                        name: item.name,
                        status: item.status,
                        date: formatIndianDate(item.dateTime),
                        rawDate: new Date(item.dateTime)
                    }));
                    setData(formatted);
    ...
    const downloadExcel = async (heading) => {
        ...
        titleCell.value = heading || "Enrollment Report";
        ...
        <div className="container-fluid mt-4">
            <div className="card shadow rounded-3">
                <div
                    ...
                    style={{ backgroundColor: "#e5e5e5", color: "#2c2727" }}
                >
                    <h4 className="mb-0">Enrollment Report</h4>
                    ...
                    <div className="d-flex align-items-center">
                        <span title="Download Excel">
                            <i
                                ...
                                onClick={() => downloadExcel("Enrollment Report")}
                            ></i>
                        </span>
                        <span title="Download CSV">
                            <i
                                ...
                                style={{ cursor: "pointer" }}
                                onClick={() => downloadCSV("Enrollment Report")}
                            ></i>
                        </span>
                    </div>
                ...
                <div className="card-body">
                    ...
                    <div className="table-responsive">
                        <table className="table table-bordered table-hover align-middle">
                            <thead>
                                <tr>
                                    {["Serial No.", "Emp ID", "Name", "Status", "Date/Time"].map((text, idx) => (
                                        <th key={idx} style={{ backgroundColor: "#e5e5e5", color: "#2c2727" }}>
                                            {text}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {currentData.length > 0 ? (
                                    currentData.map((row, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{row.empId}</td>
                                            <td>{row.name}</td>
                                            <td className={`fw-bold text-center ${row.status === "Success" ? "text-success" : "text-danger"}`}>
                                                {row.status}
                                            </td>
                                            <td>{row.date}</td>
                                        </tr>
